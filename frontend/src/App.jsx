      ...
      <div className="ml-[242px] p-3 grid grid-cols-[1fr_290px] gap-3">
        <main className="space-y-3">
          <section className="grid grid-cols-[1.25fr_1fr_1fr_1fr_1fr_1fr] gap-3">
            <HealthCard />
            <KpiCard icon="💼" title="Salário Líquido" value="€ 5.000,00" subtitle="" accent="blue" showBar />
          ...
          <section className="grid grid-cols-4 gap-3">
            <MiniTable title="Rendimentos" color="bg-emerald-700" />
            <MiniTable title="Despesas Mensais" color="bg-blue-700" />
            <MiniTable title="Dívidas" color="bg-purple-700" />
            <MiniTable title="Pagamento Ideal das Dívidas" color="bg-green-700" />
          </section>

          <section className="grid grid-cols-[320px_1fr] gap-3">
            <div className="rounded-[18px] bg-white p-4 shadow-lg border border-slate-100">
              <h3 className="font-black text-blue-900 text-sm mb-3">Saúlador: quanto tempo para ficar sem dívidas?</h3>
              <p className="text-xs font-bold">Valor disponível mensal para dívidas</p>
              <div className="mt-3 rounded-xl bg-blue-50 p-4">
                <div className="text-5xl font-black text-blue-700">23</div>
                <p className="font-black text-sm">MESES</p>
              </div>
            </div>

            <ChartBox title="Evolução da Dívida ao Longo dos Meses">
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={[
                  ...
                  <YAxis />
                  <Tooltip />
                  <Area type="monotone" dataKey="divida" stroke="#5b21b6" fill="#c4b5fd" fillOpacity={0.65} strokeWidth={4} />
                </AreaChart>
              </ResponsiveContainer>
            </ChartBox>
          </section>
  ...
  return (
    <div className="rounded-[14px] bg-white shadow-lg border border-slate-100 min-h-[120px] p-3 flex gap-3 items-center">
      <div>
        ...
        <div className="relative w-[92px] h-[92px] rounded-full bg conic-gradient flex items-center justify-center"
...
function MiniTable({ title, color }) {
  return (
    <div className="rounded-[14px] bg-white shadow-lg border border-slate-100 overflow-hidden min-h-[210px]">
      <div className={`${color} text-white text-center py-2 font-black text-xs uppercase`}>
        {title}
      </div>
      <div className="p-3 text-[11px] font-bold text-slate-700">
        <div className="grid grid-cols-3 border-b py-2">
          <span>Categoria</span><span className="text-center">Valor</span><span className="text-center">Estado</span>
        </div>
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="grid grid-cols-3 border-b py-2">
            <span>Linha {i}</span><span className="text-center">€ {i * 100}</span><span className="text-center text-green-600">OK</span>
          </div>
        ))}
      </div>
    </div>
